    ...
    renderBtn() {
        if (_.isEmpty(this.props.drinkList)) {
            if(_.isEmpty(this.props.drinkListBu)) {
                return <button type="button" className="btn btn-lg btn-light disabled ml-2 mr-2" tabIndex="-1">Tyhjennä lista</button>
            } else {
                return <button type="button" className="btn btn-lg btn-light ml-2 mr-2" onClick={() => this.handleRecovery()}>Palauta lista</button>
            }
        } else {
            return <button type="button" className="btn btn-lg btn-light ml-2 mr-2" onClick={() => this.handleEmpty()}>Tyhjennä lista</button>
        }
    }
